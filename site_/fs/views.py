...
class IndexView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['Post'] = [p for p in Post.objects.filter(status=1).all() if p.alt_text == "Innovations"][:3]
        ...
        context['Gallery_sidebar'] = Gallery.objects.filter(status=1).all()[:3]
        context['Event_sidebar'] = Event.objects.filter(status=1).all()[:3]
        ...
        context['Reviews_popular'] = Post.objects.filter(status=1,alt_text='Reviews').order_by('-hit_count_generic__hits')[:3]
        context['Articles_popular'] = Post.objects.filter(status=1,alt_text='Article').order_by('-hit_count_generic__hits')[:3]
